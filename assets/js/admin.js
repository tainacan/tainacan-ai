...
(function($) {
    ...
    const Admin = {
        ...
        bindEvents() {
            // Provider selection
            $('input[name="tainacan_ai_options[ai_provider]"]').on('change', this.handleProviderChange.bind(this));
            ...
            $(document).on('click', '.toggle-password', this.toggleApiKeyVisibility);

            // Test API (múltiplos provedores)
            $(document).on('click', '.test-api-btn', this.testProviderConnection.bind(this));
            ...
            $(document).on('click', '.tainacan-ai-dep-help', this.showInstallHelp.bind(this));

            // Field Mapping
            $('#mapping-collection-select').on('change', this.loadCollectionMetadata.bind(this));
            $('#save-metadata-mapping').on('click', this.saveMetadataMapping.bind(this));
            $('#auto-detect-mapping').on('click', this.autoDetectMapping.bind(this));
            $('#clear-mapping').on('click', this.clearMapping.bind(this));
            $('#add-mapping-row').on('click', this.addMappingRow.bind(this));
            $(document).on('click', '.remove-mapping-row', this.removeMappingRow.bind(this));

            // Modal close
            $(document).on('click', '.tainacan-ai-modal-close, .tainacan-ai-modal-overlay', this.closeModal.bind(this));
            $(document).on('click', '.tainacan-ai-modal', function(e) { e.stopPropagation(); });

            // Install tabs
            $(document).on('click', '.tainacan-ai-install-tab', this.switchInstallTab.bind(this));

            // Copy code
            $(document).on('click', '.tainacan-ai-copy-code', this.copyCode.bind(this));
        },

        initProviderSelector() {
            // Mostra a configuração do provedor selecionado
            const selectedProvider = $('input[name="tainacan_ai_options[ai_provider]"]:checked').val();
            if (selectedProvider) {
        ...
        toggleApiKeyVisibility(e) {
            ...
            const $input = targetId ? $(`#${targetId}`) : $btn.siblings('input[type="password"], input[type="text"]');
        ...
        testProviderConnection(e) {
            ...
            const $result = $(`.api-test-result[data-provider="${provider}"]`);
            const originalHtml = $btn.html();

            $btn.prop('disabled', true).html('<span class="dashicons dashicons-update spin"></span> ' + TainacanAIAdmin.texts.testing);
            $result.removeClass('success error').addClass('loading').html(
                '<span class="tainacan-ai-spinner"></span> ' + TainacanAIAdmin.texts.testing
            ).show();
            ...
            $.ajax({
                ...
                error() {
                    $result.removeClass('loading').addClass('error').html('✗ ' + TainacanAIAdmin.texts.error);
                },
                complete() {
        ...
        clearAllCache() {
            ...
            $btn.prop('disabled', true).html('<span class="dashicons dashicons-update spin"></span> ' + TainacanAIAdmin.texts.clearing);
            ...
            $.ajax({
                ...
                success(response) {
                    if (response.success) {
                        const message = typeof response.data === 'string' ? response.data : TainacanAIAdmin.texts.cacheCleared;
                        Admin.showNotice(message, 'success');
                    } else {
                        const message = typeof response.data === 'string' ? response.data : TainacanAIAdmin.texts.error;
        ...
        populateMappingCollections() {
            ...
            if ($select.length) {
                console.log('[TainacanAI] Mapping collections select found with', $select.find('option').length - 1, 'collections');
            }
        },
        ...
        loadCollectionMetadata() {
            ...
            $('#metadata-mapping-list').html('<div class="tainacan-ai-loading"><span class="tainacan-ai-spinner"></span> Carregando metadados...</div>');
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        ...
                        $('#metadata-mapping-list').html('<p class="error">Erro ao carregar metadados.</p>');
                    }
                },
                error: () => {
                    $('#metadata-mapping-list').html('<p class="error">Erro ao carregar metadados.</p>');
        ...
        renderMappingEditor() {
            ...
            existingKeys.forEach(key => {
                if (!defaultKeys.includes(key)) {
                    defaultAiFields.push({ key: key, label: key, custom: true });
                }
            });

            // Cria o seletor de metadados
            const metadataOptions = this.collectionMetadata.map(meta =>
            ...
            defaultAiFields.forEach(field => {
                ...
                const row = `
                    <div class="tainacan-ai-mapping-row" data-ai-field="${field.key}">
                ...
                if (selectedId) {
                    $container.find(`.tainacan-ai-mapping-row[data-ai-field="${field.key}"] .metadata-select`).val(selectedId);
                }
            });
        ...
        saveMetadataMapping() {
            ...
            $('.tainacan-ai-mapping-row').each(function() {
                ...
                if ($nameInput.length && $nameInput.val()) {
                    aiField = $nameInput.val().toLowerCase().replace(/\s+/g, '_');
                }

                const metadataId = $(this).find('.metadata-select').val();

                if (aiField && metadataId) {
                    const metadataName = $(this).find('.metadata-select option:selected').text();
                    mapping[aiField] = {
            ...
            $btn.prop('disabled', true).html('<span class="dashicons dashicons-update spin"></span> ' + TainacanAIAdmin.texts.saving);
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        const message = typeof response.data === 'string' ? response.data : TainacanAIAdmin.texts.saved;
                        Admin.showNotice(message, 'success');
        ...
        autoDetectMapping() {
            ...
            $btn.prop('disabled', true).html('<span class="dashicons dashicons-update spin"></span> Detectando...');
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        ...
                        Admin.showNotice(`${count} campo(s) detectado(s) automaticamente!`, 'success');
                    } else {
                        Admin.showNotice('Erro ao detectar mapeamento.', 'error');
                    }
                },
                error: () => {
                    Admin.showNotice(TainacanAIAdmin.texts.error, 'error');
        ...
        clearMapping() {
            if (!confirm('Tem certeza que deseja limpar todo o mapeamento?')) return;

            this.currentMapping = {};
            this.renderMappingEditor();
            Admin.showNotice('Mapeamento limpo. Clique em "Salvar" para confirmar.', 'success');
        },
        ...
        loadCollectionPrompt() {
            ...
            $('#collection-prompt-text').val('').attr('placeholder', TainacanAIAdmin.texts.loading || 'Carregando...');
        ...
        saveCollectionPrompt() {
            ...
            $.ajax({
                ...
                error(xhr) {
                    const message = xhr.responseJSON?.data || TainacanAIAdmin.texts.error;
                    Admin.showNotice(message, 'error');
        ...
        generatePromptSuggestion() {
            ...
            $btn.prop('disabled', true).html('<span class="dashicons dashicons-update spin"></span> ' + TainacanAIAdmin.texts.generating);
            ...
            $.ajax({
                ...
                error(xhr) {
                    const message = xhr.responseJSON?.data || TainacanAIAdmin.texts.error;
                    Admin.showNotice(message, 'error');
                    console.error('[TainacanAI] Generate suggestion error:', xhr);
                },
                complete() {
                    $btn.prop('disabled', false).html(originalHtml);
                }
        ...
        getInstallModal(depType) {
            ...
            dep.tabs.forEach((tab, index) => {
                tabsHtml += `<button class="tainacan-ai-install-tab ${index === 0 ? 'active' : ''}" data-tab="${tab.id}">${tab.name}</button>`;
                contentHtml += `<div class="tainacan-ai-install-content ${index === 0 ? 'active' : ''}" data-content="${tab.id}">${tab.content}</div>`;
            });
        ...
        closeModal(e) {
            if ($(e.target).hasClass('tainacan-ai-modal-overlay') ||
        ...
        switchInstallTab(e) {
            ...
            $(`.tainacan-ai-install-content[data-content="${tabId}"]`).addClass('active');
        },

        copyCode(e) {
            const $block = $(e.currentTarget).closest('.tainacan-ai-code-block');
